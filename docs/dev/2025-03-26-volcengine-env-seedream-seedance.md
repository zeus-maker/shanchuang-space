...
- `src/hooks/useApi.js` — 生图：火山 Base 优先 `VITE_VOLCENGINE_BASE_URL` / 默认 Ark；Key 优先 env 再回落 API 设置。视频：`createVideoTaskOnly` / `pollVideoTask` 对 1.5 Pro 走火山；Seedance 请求体仅在「火山 1.5 Pro」或「渠道为 chatfire」时用 Chatfire 侧适配，避免误配到其它渠道。
- `src/api/video.js` — `createVideoTask` / `getVideoTaskStatus` 支持额外 `headers`。
- `src/config/providers.js` — `volcengine.responseAdapter.video` 与 Chatfire 适配对齐，便于解析 URL。
