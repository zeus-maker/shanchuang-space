...
- **媒体服务**（`server/index.mjs`）新增上述路由：使用 **`@volcengine/tos-sdk`** 执行 `putObject`，对象 ACL 为 **public-read**；公网地址默认拼 **`https://${bucket}.tos-${region}.volces.com/${key}`**，可通过 **`VOLCENGINE_TOS_PUBLIC_BASE_URL`** 改为 CDN 等前缀。
