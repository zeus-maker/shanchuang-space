...
见 **`.cursor/rules/dev-delivery-zh.mdc`**：用户回复、`docs/dev` 与 commit 共用同一信息骨架（原因/修改/验证/记录），并配合 [`docs/dev/README.md`](./docs/dev/README.md)。
...
约定说明与模板见 [`docs/dev/README.md`](./docs/dev/README.md)。修复完成后需 **`git commit`**，message 须含原因、修改要点及对应 `docs/dev` 文件名（详见 `.cursor/rules/dev-delivery-zh.mdc`）。
