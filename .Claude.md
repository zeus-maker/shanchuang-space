...
- **对用户的最终说明**（原因说明、已做调整、需用户操作项等）见 Cursor 规则：`.cursor/rules/user-response-format.mdc`。  
- **仓库内修复留档与 Git commit** 见：`.cursor/rules/dev-issue-log.mdc`、`docs/dev/README.md`。

## 问题修复记录

每次完成 **Bug 修复或问题排查类** 工作后，在 **`docs/dev/`** 新增 Markdown 文件（命名：`YYYY-MM-DD-<slug>.md`），必须包含：

1. **背景与原因**（现象、根因）
2. **修改点**（文件与行为变化）
3. **复盘**（改进点、风险、后续建议）

约定说明与模板见 [`docs/dev/README.md`](./docs/dev/README.md)。修复完成后需 **`git commit`**，message 须含原因、修改要点及对应 `docs/dev` 文件名（详见 `.cursor/rules/dev-issue-log.mdc`）。
